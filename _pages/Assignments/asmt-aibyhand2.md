---
layout: assignment
permalink: Assignments/AIByHand2
title: "Written Assignment: AI By Hand - Part 2"

info:
  points: 100
  submission: "Please submit all files as a ZIP file on the Course Learning Management System."
  goals:
    - Deepen understanding of advanced AI algorithms through manual derivations.
    - Practice step-by-step reasoning with more complex problem scenarios (search, optimization, probability, and reasoning).
    - Implement hand-solved exercises in Python to validate correctness.
    - Reflect on computational, ethical, and societal implications of these algorithms at scale.
  rubric:
    - weight: 25
      description: Analysis
      preemerging: Outlines key ideas with limited accuracy or depth.
      beginning: Explains the core ideas with some detail and attempts examples.
      progressing: Provides accurate and detailed derivations with clear explanation of steps.
      proficient: Delivers comprehensive, mathematically rigorous derivations, with careful explanation of underlying principles and connections.
    - weight: 25
      description: Integration of External Sources
      preemerging: Mentions sources with minimal integration.
      beginning: References multiple credible sources with some explanation.
      progressing: Integrates authoritative sources consistently to support claims.
      proficient: Synthesizes insights from diverse, high-quality sources seamlessly into the narrative.
    - weight: 25
      description: Synthesis and Original Insight
      preemerging: Provides basic connections among ideas.
      beginning: Connects hand calculations and code to personal insights.
      progressing: Integrates theory, code, and manual calculations into a coherent whole with clear implications.
      proficient: Offers novel, insightful synthesis, connecting hands-on exercises to broader technical, ethical, and societal impacts.
    - weight: 25
      description: Organization and Writing Quality
      preemerging: Content is fragmented or unclear.
      beginning: Content is somewhat organized, with occasional lapses in clarity.
      progressing: Well-structured, coherent narrative with clear flow and correct citation.
      proficient: Polished, professional writing with excellent organization, transitions, and precise citation.

tags:
  - ai
---

## Assignment: AI By Hand - Part 2

In this assignment, you will complete **Problems 2, 5, 7, 8, 10, 11, 12, and 13** of the *AI by Hand* exercises. This set focuses on **more advanced AI concepts**, requiring both manual derivation and implementation in code.

### Instructions

1. Download and review the full *AI by Hand - Vol 1* book here:  
   [AI by Hand - Vol 1 (Scribd)](https://www.scribd.com/document/726922630/AI-by-Hand-Vol-1)

2. Complete the following specific parts from the text (linked below):
   - [Problem 2](https://lnkd.in/gDANw4iH)  
   - [Problem 5](https://lnkd.in/gwe69_84)  
   - [Problem 7](https://lnkd.in/gsiU2uc2)  
   - [Problem 8](https://lnkd.in/g39jcD7j)  
   - [Problem 10](https://lnkd.in/gyKzNGDy)  
   - [Problem 11](https://lnkd.in/gDW8Um4W)  
   - [Problem 12](https://lnkd.in/g4KHF-Hd)  
   - [Problem 13](https://lnkd.in/g2rM9iV2)

3. For each problem:
   - **By hand**: Work through all derivations and computations step-by-step. Show your complete work (e.g., calculations, intermediate steps, hand-written notes).
   - **In code (Python recommended)**: Implement the algorithms or calculations in Python to verify correctness and provide additional insights.
   - **Write-up**: Provide a narrative that explains your process, highlights connections between hand-computation and implementation, and reflects on what you learned.

4. Your final submission should include:
   - Scans or photographs of your **handwritten solutions**.
   - A **Python notebook or code file** reproducing the exercises.
   - A **written report (1–2 pages)** containing:
     - Introduction and overview of these Problems.
     - Step-by-step derivations and explanations.
     - Verification with code outputs.
     - Reflection on computational trade-offs and ethical considerations in AI.

---

## Reflection Prompts

- Which exercise did you find most challenging to complete by hand, and why?  
- Did coding the exercises help reveal subtleties that were harder to see on paper?  
- How do these small-scale hand exercises relate to large-scale AI architectures like **Transformers**?  
- What ethical issues emerge when the techniques are scaled up in real-world applications?  
- How do resource usage and sustainability concerns influence the practicality of large-scale AI?

---

## Resources

- Full *AI by Hand* book: [AI by Hand - Vol 1](https://www.scribd.com/document/726922630/AI-by-Hand-Vol-1)
- Individual parts for this assignment:
  - [Part 2](https://lnkd.in/gDANw4iH)
  - [Part 5](https://lnkd.in/gwe69_84)
  - [Part 7](https://lnkd.in/gsiU2uc2)
  - [Part 8](https://lnkd.in/g39jcD7j)
  - [Part 10](https://lnkd.in/gyKzNGDy)
  - [Part 11](https://lnkd.in/gDW8Um4W)
  - [Part 12](https://lnkd.in/g4KHF-Hd)
  - [Part 13](https://lnkd.in/g2rM9iV2)
